...
extension StringProtocol {
  var caseFolded: LazySequence<FlattenSequence<LazyMapSequence<LazySequence<Self.UnicodeScalarView>.Elements, [Unicode.Scalar]>.Elements>> {
    unicodeScalars.lazy.map { _caseFoldingData[$0] ?? [$0] }.joined()
  }
  
  var caseFoldedString: String {
    String(caseFolded)
  }
}
...
enum Status: String, CaseIterable {
  ...
  struct Parser: CustomRegexComponent {
    typealias Match = Status
    
    func match(_ input: String, startingAt index: String.Index, in bounds: Range<String.Index>) -> (upperBound: String.Index, match: Status)? {
      guard index < bounds.upperBound else { return nil }
      
      let upperBound = input.index(after: index)
      switch input[index] {
      case "C": return (upperBound, .common)
      case "F": return (upperBound, .full)
      default: return nil
      }
    }
  }
  
  static var parser: Parser { Parser() }
}

public let _caseFoldingData: [Unicode.Scalar: [Unicode.Scalar]] = {
  let separator = Regex {
    ";"
    zeroOrMore(.whitespace, .reluctantly)
  }
  ...
  let lineRegex = Regex {
    ...
    capture(oneOrMore(.hexDigit)) {
      UnicodeScalar(UInt32($0, radix: 16)!)!
    }
    separator
    
    // Only use foldings with common (C) or full (F) status
    Status.parser
    separator
    
    // The mapping is one or more space-separated scalars -- capture and
    // convert to an array of scalars
    capture {
      oneOrMore {
        choiceOf {
      ...
      $0.split(separator: " ")
        .map { UnicodeScalar(UInt32($0, radix: 16)!)! }
    }
    separator
  ...
  return Dictionary(
    uniqueKeysWithValues: CaseFolding_txt
      .split(separator: "\n")
      .lazy
      .compactMap{ $0.firstMatch(of: lineRegex)?.result }
      .map { _, scalar, caseFolded in (scalar, caseFolded) }
  )
}()
